...
7.  Give a careful proof of Corollary 2.2.3: A graph is a forest if and only if there is at most one path between any pair of vertices. Use proof by contrapositive (and not a proof by contradiction) for both directions.
    - Prove that if there exists a pair of vertices with more than one path between them, then a graph $G$ is not a forest:
      - Assume that there is a set of vertices $v$ and $u$ in graph $G$, and this set of vertices has at least 2 distinct paths between them.
      - This means that there are at least points $u'$ and $v'$ where the points diverge and where they come back together. 
      - If we merge these two paths using a union, this creates a clear cycle. 
    - Prove now that that if $G$ is not a forest, there exists a pair of vertices with more than one path between them:
      - Assume that $G$ is not a forest
      - Since $G$ is not a forest, it contains at least one cycle
       - A cycle has a clockwise and a counterclockwise direction, meaning one could go from vertex to vertex in either direction.
       - This means that the pair of vertices included in the guaranteed cycle contain more than one path between them.
     - ![Here is a visual, for demonstration](../visuals/2.2_exercise8.png)